...
bool bluetooth_get_host_address(std::string &out_address)
{
	...
	if (x_cachedHostBluetoothAddress.length() == 0)
	{
		...
		{
			HSL_LOG_INFO("bluetooth_get_host_address") << "Found a bluetooth radio";
		}
		else
		{
			HSL_LOG_ERROR("bluetooth_get_host_address") << "Failed to find a bluetooth radio";
			bSuccess = false;
		}
		...
		if (bSuccess)
		{
			...
			{
				HSL_LOG_INFO("bluetooth_get_host_address") << "Retrieved radio info";
				out_address = bluetooth_address_to_string(&radioInfo.address);
				x_cachedHostBluetoothAddress= out_address;
			}
			else
			{
				HSL_LOG_ERROR("bluetooth_get_host_address")
	...
	{
		HSL_LOG_INFO("bluetooth_get_host_address") << "Using cached radio info";
