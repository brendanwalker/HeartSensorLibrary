...
bool PlatformDeviceAPIWin32::startup(IDeviceHotplugListener *broadcaster)
{
	...
	if (g_hWnd == nullptr)
	{
		...
		{
			HSL_LOG_ERROR("DeviceHotplugAPIWin32::startup") << "Could not create message window!";
			bSuccess = false;
		}
	}
	else
	{
		HSL_LOG_WARNING("DeviceHotplugAPIWin32::startup") << "Message handler window already created";
	}
	...
	if (bSuccess)
	{
		...
		if(!SUCCEEDED(hr))
		{
			HSL_LOG_ERROR("DeviceHotplugAPIWin32::startup") << "Failed to initialize MS Media Foundation!";
			bSuccess = false;
		}
	}
...
static HDEVNOTIFY register_device_class_notification(HWND__* hwnd, const GUID &guid)
{
	...
	if (dev_notify == nullptr)
	{
		HSL_LOG_ERROR("RegisterDeviceClassNotification") << "Could not register for device notifications!";
	}
