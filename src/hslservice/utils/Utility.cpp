...
namespace Utility
{
	...
	std::wstring convert_string_to_wstring(const std::string& string)
	{
		std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> conv;
		return conv.from_bytes(string);
	}

	std::string convert_wstring_to_string(const std::wstring& wstring)
	{
		std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> conv;
	...
	void copyCString(const char *source, char* target, size_t target_size)
	{
