...
void WorkerThread::startThread()
{
    if (!m_threadStarted)
    {
        ...
        HSL_LOG_INFO("WorkerThread::start") << "Starting worker thread: " << m_threadName;
		onThreadStarted();
...
void WorkerThread::stopThread()
{
    if (m_threadStarted)
    {
        if (!m_exitSignaled)
        {
            HSL_LOG_INFO("WorkerThread::stop") << "Stopping worker thread: " << m_threadName;
            ...
            HSL_LOG_INFO("WorkerThread::stop") << "Worker thread stopped: " << m_threadName;
			onThreadHaltComplete();
        }
        else
        {
            HSL_LOG_INFO("WorkerThread::stop") << "Worker thread already stopped: " << m_threadName;
        }
